...
| 2 | Autenticação e autorização | a fazer |
...
```bash
cd backend
./.venv/bin/python -m tests.test_esquema
```

Valida as restrições do esquema com dados reais (um padrinho em várias crianças,
apadrinhamento entre cidades, unicidade de cartões e kits, filtros de acesso).
Não deixa nada gravado na base.
...
> Ainda não existe `app/main.py` — chega na fase 2.
